...
Setting up some the response parameters (PK properties and GR efficacy values)
...
Defining the experiment (duration, dosages)
...
for (id in seq_along(Doses)) {
  # calculate the concentration profile for each dosage
  ...
  # calculate the relative growth rate over time
  relk <- relk_over_time(GR_para, conc_profile)
  # average the relative growth rate over time to get the predicted TGR
  TGR_predicted[id] <- GR_inVitro_integration(conc_profile, GR_para, int_method = 'mean')

  # generate the variables for plotting
