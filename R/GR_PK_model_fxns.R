...
PK_to_conc_profile <-
  function(PK_para,
    ...
    conc <-
      function(t)
        (consts[1] * eigvec[2, 1] * exp(eigval[1] * (t)) + consts[2] * eigvec[2, 2] *
           exp(eigval[2] * (t)))
    ...
    serum_conc <-
      function(t)
        gsignal::pulstran(t, d, pp, 1 / .integration_step())
    return(list(
      Time = tp,
      Conc = serum_conc,
...
#' @param GR_para  numeric array for the in vitro GR parameters of the drug 
#' (required fields: \code{GR_inf}, \code{GEC50}, \code{h_GR})
#'
...
relk_fct <- function(c, GR_para) {
  GR_c <-
    logistic_4parameters(pmax(0, c), GR_para$GR_inf, 1, GR_para$GEC50, GR_para$h_GR)
  return(log2(GR_c + 1))
}
...
GR_inVitro_integration <-
  function(conc_profile, GR_para, int_method = "mean") {
